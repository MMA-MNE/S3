...
def load_data():
   # Load the data
    ...
    df['transfer_in'].replace('n','N', inplace = True)
    df['regimen'].replace('i','I', inplace = True)
    df['regimen'].replace('r','R', inplace = True)
    df['regimen'].replace('c','C', inplace = True)
    df['regimen'].replace('I ','I', inplace = True)
    df['regimen'].replace(' I','I', inplace = True)
    df['Xpert'].replace('n','N', inplace = True)
    df['Xpert'].fillna('Not test', inplace = True)
    df['nationality'].replace('','N', inplace = True)
    df['nationality'].replace('n','N', inplace = True)
    df['nationality'].replace('F','N', inplace = True)
    df['nationality'].replace('M','N', inplace = True)
    df['smoking'].replace('n','N', inplace = True)
    df['smoking'].replace('p','P', inplace = True)
    df['smoking'].replace('c','C', inplace = True)
    df['smoking'].replace('n','N', inplace = True)
    df['smoking'].replace('N ','N', inplace = True)
    df['smoking'].replace(' N','N', inplace = True)
    df['outcome'].replace('n','N', inplace = True)
    df['outcome'].replace('c','C', inplace = True)
    df['pt_type'].replace('n','N', inplace = True)
    df['pt_type'].replace('r','R', inplace = True)
    df['dm'].replace('n','N', inplace = True)
    df['dm'].replace('y','Y', inplace = True)
...
if df.empty:
    ...
    #tsp dataframe
    df_tsp= df.groupby(['tsp'],as_index=False).agg({'year':'count'})
    ...
    #Tsps Bar Plot
    fig_tsp= px.bar(df_tsp, x = 'tsp', y= 'year', labels={'year':'Patients'},
                        title='Township-wise Case Holdings', color='tsp', 
                             color_discrete_sequence=px.colors.qualitative.Pastel)

    st.plotly_chart(fig_tsp)
    ...
    st.plotly_chart(fig_age_group_detail)
    ...
    percent_hiv_known= count_hiv_known['year']/(count_hiv_known['year'] + 
                                                count_hiv_unknown['year'])*100
    ...
    st.plotly_chart(fig_hiv_known_percent)

    colors= {'Known':'yellow','Unk':'red'}
    fig_hiv_known = px.pie(df_hiv_known,names= 'HIV',values='year',labels={'year':'Patients'},title='HCT testing',
    ...
    st.plotly_chart(fig_percent_art)
    ...
    st.plotly_chart(fig_percent_cpt)
    ...
    df_gxp_test['Xpert'].replace({'I':'Tested','N':'Tested','RR':'Tested', 'T':'Tested', 'TI':'Tested'}, inplace=True)
    df_gxp_test= df_gxp_test.groupby(['Xpert'],as_index=False).agg({'year': 'sum'})

    count_gxp_test= df_gxp_test.iloc[1]
    count_gxp_nottest= df_gxp_test.iloc[0]
    ...
    st.plotly_chart(fig_gxp_test_percent)


    colors= {'Tested':'blue','Not test':'grey'}
    fig_gxp_test = px.pie(df_gxp_test, names= 'Xpert',values='year',labels={'year':'Patients'},title='GXP Testing Percent in GXP Eligible cases',
                  color= 'Xpert',color_discrete_map= colors, hole= 0.5)


    #st.plotly_chart(fig_gxp_test)

    colors= {'I':'blue','N':'grey','T':'red','Not test':'black','RR':'orange','TI':'yellow'}
    ...
    df_dm_testing = df_dm_testing.groupby(['dm'], as_index= False).agg({'year':'sum'})

    count_dm_tested= df_dm_testing.iloc[0]
    count_dm_unk= df_dm_testing.iloc[1]
    ...
    st.plotly_chart(fig_dm_tested_percent )




    colors= {'Tested':'blue','Unknown':'grey'}
    fig_dm_testing = px.pie(df_dm_testing, names= 'dm',values='year',labels={'year':'Patients','dm':'DM'},
                        title='DM Testing in 40 yrs and above TB Patients',
                  color= 'dm',color_discrete_map= colors, hole= 0.6)
    #st.plotly_chart(fig_dm_testing)
    ...
    fig_dm.update_layout(xaxis_title= 'Patients',yaxis_title= 'DM screening')
    st.plotly_chart(fig_dm)
    ...
    st.plotly_chart(fig_tsr_percent)
    ...
    fig_tsr = px.pie(df_tsr, names= 'outcome',values='year',labels={'year':'Patients','outcome':'TSR'},
                            title='TSR of TB cases from previous year',
                    color= 'outcome',color_discrete_map= colors, hole= 0.6)
    #st.plotly_chart(fig_tsr)

    #Tx outcome chart
    colors= {'Cure':'blue','Died':'black','Failure':'grey','Loss to follow up':'orange','Moved to second line':'purple',
